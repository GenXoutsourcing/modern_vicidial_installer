...
echo "ts,agents_ready,agents_incall,agents_paused,agents_queue,live_calls,calls_sip1,calls_sip2,calls_sip3,hopper,threads_conn,threads_run,slow_q,load_sip1,load_sip2,load_sip3,load_db1,genx_rss_mb,health_ms,lt_calls,lt_drops" > "$CSV"
...
while true; do
  ...
  LINE="$TS,${READY:-0},${INCALL:-0},${PAUSED:-0},${QUEUE:-0},${CT:-0},${C1:-0},${C2:-0},${C3:-0},${HOPPER:-0},${TCONN:-0},${TRUN:-0},${SLOW:-0},${L1:-0},${L2:-0},${L3:-0},${LDB:-0},${RSS:-0},${HMS:-0},${LTCALLS:-0},${LTDROPS:-0}"
  echo "$LINE" >> "$CSV"
  echo "$TS agents R/I/P/Q=${READY:-0}/${INCALL:-0}/${PAUSED:-0}/${QUEUE:-0} calls=${CT:-0} (s1=${C1:-0} s2=${C2:-0} s3=${C3:-0}) hopper=${HOPPER:-0} db=${TCONN:-0}c/${TRUN:-0}r slow+${SLOW:-0} load s1/s2/s3/db1=${L1:-0}/${L2:-0}/${L3:-0}/${LDB:-0} genx=${RSS:-0}MB health=${HMS:-0}ms LT calls=${LTCALLS:-0} drops=${LTDROPS:-0}"
  sleep 10
done
