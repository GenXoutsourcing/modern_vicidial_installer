...
install_patched_string_crc() {
    if perl -MString::CRC -e 1 >/dev/null 2>&1; then
        return 0
    fi
    local build_dir=/usr/src/String-CRC-1.0
    rm -rf "$build_dir"
    cd /usr/src
    copy_asset String-CRC-1.0-alma10.tar.gz
    tar xzf String-CRC-1.0-alma10.tar.gz
    cd "$build_dir"
    perl Makefile.PL
    make
    make install
    perl -MString::CRC -e 1
}

install_vicidial_keepalive_guard() {
    copy_asset vicidial-singleton-keepalive /usr/local/bin/vicidial-singleton-keepalive
    chmod 755 /usr/local/bin/vicidial-singleton-keepalive
}
...
python3 -m pip install --no-index --find-links "$ASSET_DIR/python-wheels" --break-system-packages mysql-connector-python || python3 -m pip install --no-index --find-links "$ASSET_DIR/python-wheels" mysql-connector-python
...
#  7, E and C are managed by /usr/local/bin/vicidial-singleton-keepalive on EL10
#     because EL10 truncates the ps output used by ADMIN_keepalive_ALL.pl for these long commands.
...
* * * * * /usr/local/bin/vicidial-singleton-keepalive
