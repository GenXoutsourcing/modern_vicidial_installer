...
dtmfVolume = 0.15
dtmf0Audio.volume = dtmfVolume;
dtmf1Audio.volume = dtmfVolume;
dtmf2Audio.volume = dtmfVolume;
dtmf3Audio.volume = dtmfVolume;
dtmf4Audio.volume = dtmfVolume;
dtmf5Audio.volume = dtmfVolume;
dtmf6Audio.volume = dtmfVolume;
dtmf7Audio.volume = dtmfVolume;
dtmf8Audio.volume = dtmfVolume;
dtmf9Audio.volume = dtmfVolume;
dtmfHashAudio.volume = dtmfVolume;
dtmfStarAudio.volume = dtmfVolume;
...
window.addEventListener('message', function(event) {
	if (!event || !event.data || event.data.type !== 'GENX_VICIPHONE_MUTE') {
		return;
	}
	if (uiElements && uiElements.mic_mute) {
		uiElements.mic_mute.click();
	}
	if (event.source && event.source.postMessage) {
		event.source.postMessage({
			type: 'GENX_VICIPHONE_MUTE_STATE',
			muted: muted,
			incall: incall
		}, event.origin || '*');
	}
